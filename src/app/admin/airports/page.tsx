...
export default function AirportsPage() {
  ...
  const fetchAirports = async () => {
    try {
      ...
      setError('Ошибка при загрузке аэропортов');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: '', city: '', country: '', iata_code: '', icao_code: '' });
    } catch (err: any) {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/airport/${editingAirport.airport_id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(formData)
      });
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Вы уверены, что хотите удалить этот аэропорт?')) return;
    ...
    return (
      <div className="min-h-screen bg-gray-100 p-6">
        <div className="text-center">
          ...
          <p className="text-gray-600">Загрузка аэропортов...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-3xl font-bold text-gray-900">Управление аэропортами</h1>
        </div>
        <button
      ...
      {(showCreateForm || editingAirport) && (
        <div className="bg-white rounded shadow p-6 mb-6">
          <h2 className="text-xl font-bold mb-4">
            {editingAirport ? 'Редактировать аэропорт' : 'Добавить новый аэропорт'}
          </h2>
          <form onSubmit={editingAirport ? handleUpdate : handleCreate} className="space-y-4">
